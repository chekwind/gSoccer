...
@author: CC
'''
from app.share.dbopear import dbFriendTop
from app.chatServer.core.chat.ChatContextManager import ChatContextManager
class ChatLog():
		...
		if not id in self.friendList:#此角色不在好友列表中
			...
			if len(self.friendList)>self.playercount:#最近联系人数量大于联系人数量上限
				...
				if len(xxList)<1:#如果都在线的话
					...
					self.friendList.remove(xxList[0])#删除不在线中的第一个角色
