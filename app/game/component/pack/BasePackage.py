...
class BasePackage(object):
		...
		for fromItem in self._items:
			if position == fromItem['position']:
				if stack==0:
					...
					return
				fromItem['itemComponent'].pack.updateItemStack(stack)
	...
	def getItemByVirtualPosition(self,position,page):
		'''根据虚拟的坐标和包裹分页获取物品'''
		position=self.getRealPosition(position,page)
		item=self.getItemByPosition(position)
		return item

	def getRealPosition(self,position,page):
		'''获取物品在包裹里的真实位置'''
		realPosition=position+(page-1)*30
		return realPosition

	def getVirtualPosition(self,position):
		'''获取虚拟的坐标与分页'''
		page=(position/self.limit)+1
		vposition=position%self.limit
		return page,vposition
